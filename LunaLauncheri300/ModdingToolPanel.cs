...
{
    public partial class ModdingToolPanel : Form
    {
        ...
        private void ModdingToolPanel_Load(object sender, EventArgs e)
        {
            LoadModdingTool();
        ...
        private void LoadModdingTool()
        {
            try
            {
                // Set the path to the ModdingTool executable
                string moddingToolPath = Environment.ExpandEnvironmentVariables("%IG_ROOT%i30/Utilities/ModdingTool/ModdingTool.exe");
                
                // Verify the ModdingTool executable exists
                if (!File.Exists(moddingToolPath))
                {
                    MessageBox.Show($"The ModdingTool executable was not found at: {moddingToolPath}");
                    return;
                }
        ...
        private void ModdingToolPanel_FormClosing(object sender, FormClosingEventArgs e)
        {
